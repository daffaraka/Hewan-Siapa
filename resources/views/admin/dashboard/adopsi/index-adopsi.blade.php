                 ...
                 <table class="table"style="background-color: white; box-shadow:grey 1px 1px 2px;">
                   ...
                   <a button type="button" class="btn btn-primary" style="margin-bottom: 10px; " href="{{route('adopsi.create')}}"> Buat Adopsi</a> <div class="col-md-12">
                        ...
                        <tbody>
                            ...
                            @foreach ($adopsi as $listAdopsi)
                                <tr>
                                    <td> <img class="card-img-top" src=" {{asset('storage/post/adopsi/'.$listAdopsi->nama_post_adopsi.'-'.$listAdopsi->image_post_adps)}}"
                                         style="height: 100px; max-width:100px; border-radius:50%; object-fit:cover; object-position:center;"> </td>
                                    <td>{{$listAdopsi->nama_post_adopsi}}</td>
                                    <td>Admin</td>
                                    <td>{{$listAdopsi->nama_jenis_hewan}} -- {{$listAdopsi->nama_ras_hewan}}</td>
                                    <td>{{$listAdopsi->lokasi_post_adps}}</td>
                                    <td>{{$listAdopsi->created_at}}</td>
                                    <td> 
                                        <a href="{{ route('adopsi.edit',['id' => $listAdopsi->id]) }}" class="btn-sm btn-warning">Edit</a>
                                        <a href="{{ route('adopsi.delete',['id' => $listAdopsi->id]) }}" class="btn-sm btn-danger">Delete</a> </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
